...
pertfiles <- list("0503PVFulldata.csv", "0503PHFulldata.csv", "0503SFFulldata.csv", "0503SOFulldata.csv")
sitesdata <- lapply(pertfiles,read.csv)
names(sitesdata) <- sites


locandclim <- read.csv("LocandClim_sa.csv")
...
for (sitedata in GrowthRates){
  ...
  ##ElevDist
  elevdist_plot <- ggplot(sitedata, aes(ElevDist)) + geom_vline(xintercept=0, color="red") + 
    geom_histogram(binwidth=0.05) + labs(title = paste("ElevDist by Family at", sites[iter]), x="ElevDist")
...
latlong <- ggplot(locandclim, aes(y=Latitude, x=Longitude)) + geom_point() +
  ...
  geom_text(data=cg_locandclim, label=c("SO", "SF", "PY", ""), hjust=-1, color = "red") + xlab("Longitude (ºE)") + ylab("Latitude (ºN)")

##Elevation
longelev <- ggplot(locandclim, aes(y=Elevation, x=Longitude)) + geom_point() +
  geom_point(data=cg_locandclim, shape=17, size = 3, color = "white") + 
  geom_text(data=cg_locandclim, label=c("SO", "SF", "", ""), hjust=-1, color = "red") + xlab("Longitude (ºE)") + ylab("Elevation (m)")

#MSP
longmsp <- ggplot(locandclim, aes(y=MSP, x=Longitude)) + geom_point() +
  geom_point(data=cg_locandclim, shape=17, size = 3, color = "white") + 
  geom_text(data=cg_locandclim, label=c("SO", "SF", "", ""), hjust=-1, color = "red") + xlab("Longitude (ºE)") + ylab("Mean Summer Precipitation (mm)")
...
latlongelev <- ggplot(locandclim, aes(y=Latitude, x=Longitude, color=Elevation)) + geom_point() +
  ...
  geom_text(data=cg_locandclim, label=c("SO", "SF", "PY", ""), hjust=-1, color = "red") + xlab("Longitude (ºE)") + ylab("Latitude (ºN)") + scale_colour_gradient(low = "white", high = "black")
...
```{r}
##Now you have gr_mod and mort_mod
