---
title: "Analysis Draft 3 Multiple Nonlinear Regression"
author: "RPK"
date: "March 25, 2021"
...
setwd("../Basic Datasets") #easier directory to work from
...
The most thorough way to do this is to iterate through every possible combination of variables, look at the AIC values, and pick the ones that are within 2 of the best one
- to start out, maybe just iterate through all the options and keep all the AIC values
--then pick the best ones
---then store the R2 and parameter values for the best ones
---create average model from those options
...
Should be able to save yourself a headache by combining the two. The problem is the variance structure is different between the two 
...
for (gstat in c(1,2)){
  ...
  write.csv(GR_AICmat, print(paste("040221_", DepVar_name, "_BestMultReg.csv", sep='')))
...
write.csv(Mort_AICmat, "033121MortalityBestMultReg.csv")
