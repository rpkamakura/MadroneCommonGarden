...
#setwd("Madrone4/Madrone3/Basic Datasets") #easier directory to work from
...
pertfiles <- list("../02Lvl1Processed/033121PHFulldata.csv", "../02Lvl1Processed/033121PVFulldata.csv", "../02Lvl1Processed/033121SFFulldata.csv", "../02Lvl1Processed/033121SOFulldata.csv")
...
Visualization tips
- to look at the source level differences:
  - make the X axis grouped by source with the y being mortality and color-coded by ecoregion (highlighting most local)
    - maybe order them by highest mortalty rate
    
- do the same thing with all the rest. Then you should be able to see potential trends and look at the individual sources to compare. You can also look for potential drivers of trends and identify specific families
...
SF_bFFP <- ggplot(SF, aes(x=bFFP, y=TotGrowth)) + 
  geom_point(size = 3) + 
  geom_smooth(method="lm",formula = y ~ x + I(x^2), color='grey')
...
SO_bFFP <- ggplot(SO, aes(x=bFFP, y=TotGrowth)) + 
  geom_point(size = 3) + 
  geom_smooth(method="lm",formula = y ~ x + I(x^2), color='grey')
...
fit_tg <- aov(TotGrowth ~ Site, data=totData)
  ...
  ##bffp_func <- function(x){-0.001*x^2 + 0.0467*x}
  #curve(bffp_func, 0, 180)
...
##SF - Ecoregion + Dist + ElevDist + bFFP
SF <- GrowthRates[[3]]
SF$Ecoregion <- relevel(as.factor(SF$Ecoregion), ref="WIVA")
SF$ElevDist2 <- (SF$ElevDist)^2
SF$bFFP2 <- (SF$bFFP)^2

SF_reg <- lme(TotGrowth ~ Ecoregion + Dist + ElevDist + ElevDist2 + bFFP + bFFP2, random = ~1|Block, data = SF, na.action = na.omit, control = ctrl, method="ML")
...
#PV - Ecoregion + Dist + ElevDist + bFFP
PV <- GrowthRates[[2]]
PV$Ecoregion <- relevel(as.factor(PV$Ecoregion), ref="PULO")
...
PV_reg2 <- lme(TotGrowth ~ Ecoregion + Dist + ElevDist + ElevDist2 + bFFP + bFFP2, random = ~1|Block, data = PV, na.action = na.omit, control = ctrl, method="ML")
...
SO_reg2 <- lme(TotGrowth ~ Ecoregion + bFFP + bFFP2, random = ~1|Block, data = SO, na.action = na.omit, control = ctrl, method="ML")
...
```{r}
sites <- c("PH","PV", "SF", "SO")
condata <- IndivCond_final(sitesdata, sites) ##need to fix this function (is not in PerSiteAnalys2)
# totMData <- rbindlist(condata)
# 
# Overall_Mort <- ggplot(totMData, aes(x=Site, y=(PercDead)*100)) + 
#   geom_boxplot() + scale_x_discrete(name = "Common Garden Site") +
...
#   
###IF YOU WANT IT TO MATCH UP YOU NEED TO DO RESCALING
...
write.csv(storage_mat, "060821mortality_estimates.csv")
...
##PH - Ecoregion + ElevDist + bFFP
PH_mort <- condata[[1]]
PH_mort$Ecoregion <- relevel(as.factor(PH_mort$Ecoregion), ref="PULO")

PH_reg <- glmer(PercDead ~ Ecoregion + ElevDist + bFFP + (1|Block), data = PH_mort, 
                family="binomial", control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)), 
                weights = Total)
...
##SF - Ecoregion + Dist + Aspect + bFFP
SF_mort <- condata[[3]]
SF_mort$Ecoregion <- relevel(as.factor(SF_mort$Ecoregion), ref="WIVA")

SF_reg <- glmer(PercDead ~ Ecoregion + Dist + Aspect + bFFP + (1|Block), data = SF_mort, 
                family="binomial", control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)), 
                weights = Total)
...
##PV - Ecoregion + Dist + Slope + Aspect + bFFP
PV_mort <- condata[[2]]
PV_mort$Ecoregion <- relevel(as.factor(PV_mort$Ecoregion), ref="PULO")

PV_reg2 <- glmer(PercDead ~ Ecoregion + Dist + Slope + Aspect + bFFP + (1|Block), data = PV_mort, 
                family="binomial", control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e4)), 
                weights = Total)
...
#   geom_smooth(method="lm", color='grey')
# 
# Phen_bFFP <- ggplot(PhenData, aes(x=bFFP, y=Bud.Elongating)) + 
#   geom_point(size = 3) + 
#   geom_smooth(method="lm", color='grey')
# 
...
##BSW
#BSW_reg <- lme(Bud.Swollen ~ Ecoregion + Lat + Long + bFFP + MSP + Slope + Aspect, random = ~1|Block, data = PhenData, na.action = na.omit, control = ctrl, method="ML")
...
##LEM
#LEM_reg <- lme(Leaves.emerging ~ Ecoregion + Lat + Long + Elev + bFFP + MSP + Slope + Aspect, random = ~1|Block, data = PhenData, na.action = na.omit)
...
#totData
#totMData
colnms <- c("Ecoregion", "Source", "PH_OGR","PV_OGR","SF_OGR", "SO_OGR", "PH_DBK", "PV_DBK",
            "SF_DBK", "SO_DBK", "PH_PMT", "PV_PMT", "SF_PMT", "SO_PMT")
...
write.csv(outputmat, "0701SourceAverages.csv")
...
for (st in sites){ #site loop
  ...
  write.csv(sig05_pair, print(paste("GRByFamilyTotGrowth_", st, ".csv", sep="")))
  ...
  write.csv(sig05_pair, print(paste("GRByFamilyDieback_", st, ".csv", sep="")))
  ...
  write.csv(sig05_pair, print(paste("GRByFamilyMort_", st, ".csv", sep="")))
}
...
#scale functions
#silhouette method 
fullClim <- read.csv("../02Lvl1Processed/WeatherData.csv")
locData <- read.csv("../02Lvl1Processed/033121LocClim.csv")
ecoregdat <- read.csv("../02Lvl1Processed/EcoregionData.csv")
...
for (s in 1:4){ #sites
 ...
 # o_cl_mort_se <- sd(na.omit(site_Mort$PercDead[site_Mort$Family %in% fams_OClust]))/sqrt(length(na.omit(site_Mort$PercDead[site_Mort$Family %in% fams_OClust])))
  ...
  write.csv(gr_df, print(paste("../02Lvl1Processed/060721GrowthClust_",sites[s], ".csv", sep="")))
  write.csv(dbk_df, print(paste("../02Lvl1Processed/060721DBKClust_",sites[s], ".csv", sep="")))
  write.csv(mort_df, print(paste("../02Lvl1Processed/060721MortClust_",sites[s], ".csv", sep="")))
}

write.csv(p_val_df, "../02Lvl1Processed/060821LocalDistant_pvals.csv")
```
...
p_val_df <- read.csv("../02Lvl1Processed/060821LocalDistant_pvals.csv")
...
p_dat <- as.data.frame(aggregate(PhenData[,11], list(PhenData$Name), mean))
names(p_dat) <- c("Family", "BE")
p_dat$BE2 <- (p_dat$BE)^2
...
for (s in c(1:4)){
  ...
  gr_p <- na.omit(merge(gr, p_dat, by="Family"))
  mort_p <- na.omit(merge(mort, p_dat, by="Family"))
  
  ##so you know where you are
  print(paste("Data Analysis for", st))
  
  ##Linear Regression
  lm_gr <- lme(TotGrowth ~ BE2 + BE, random = ~1|Block, data = gr_p, na.action = na.omit, control = ctrl, method="ML")
  #lm_gr <- lm(TotGrowth ~ BE2 + BE, data = gr_p)
  #print(summary(lm_gr))
  reg_sum <- summary(lm_gr)
  reg_r2 <- r.squaredGLMM(lm_gr) 
  reg_int <- intervals(lm_gr, which = "fixed")
  print(reg_r2)
  print(reg_int)
  
  lm_dbk <- lme(AmntDieback~ BE2 + BE, random = ~1|Block, data = gr_p, na.action = na.omit, control = ctrl, method="ML")
  #lm_dbk <- lm(AmntDieback ~ BE2 + BE, data = gr_p)
  #print(summary(lm_dbk))
  reg_sum <- summary(lm_dbk)
  reg_r2 <- r.squaredGLMM(lm_dbk) 
  reg_int <- intervals(lm_dbk, which = "fixed")
  print(reg_r2)
  print(reg_int)
  
  lm_mort <- glmer(PercDead ~ BE2 + BE + (1|Block), data = mort_p, family="binomial", 
                   control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)), weights = Total)
  #lm_mort <- lm(PercDead ~ BE2 + BE, data = mort_p)
  m_summ <- summary(lm_mort)
  reg_r2 <- r.squaredGLMM(lm_mort)
  reg_int <- m_summ$coefficients
...
write.csv(sto_mat,"040621Dieback_intervals.csv")
